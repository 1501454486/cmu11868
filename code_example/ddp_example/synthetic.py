import os
import time
from project.run_sentiment_linear import BATCH
import torch
...
def train_ddp(dataset, batch_size = 1024, num_epochs = 100):
    ...
    ddp_model = DDP(model, device_id = [device_id])
    
    loss_fn = nn.MSELoss()
    optimizer = optim.SGD(ddp_model.parameters(), lr = 0.001)
    
    # We sample isomg DistributedSampler such that each GPU gets unique data
