...
class Attn_Softmax(Function):
    ...
    def backward(ctx: Context, out_grad: Tensor) -> Tuple[Tensor, Tensor]:
      #   BEGIN ASSIGN4_1_2
      soft_inp, mask = ctx.saved_values[0], ctx.saved_values[1]
      grad_inp = soft_inp.f.attn_softmax_bw(out_grad, soft_inp)
      grad_mask = zeros(mask.shape, mask.backend)
...
class LayerNorm(Function):
    ...
    def backward(ctx: Context, out_grad: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
      #   BEGIN ASSIGN4_2_2
      inp, gamma, beta, vars, means = ctx.saved_values[0], ctx.saved_values[1], ctx.saved_values[2], ctx.saved_values[3], ctx.saved_values[4]
      return inp.f.layernorm_bw(out_grad, inp, gamma, beta, vars, means)
      #   END ASSIGN4_2_2
